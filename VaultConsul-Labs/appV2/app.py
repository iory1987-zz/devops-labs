...
from tools import HealthVault, HealthConsul, HealthMongoDB, HealthRabbitMQ, GetCredentials
...
class MainClass(Resource):
    ...
    def post(self):
        ...
        try:
            credentials = PlainCredentials(config[1]['rabbit']['user'], config[1]['rabbit']['pass'])
            parameters  = ConnectionParameters(config[1]['rabbit']['host'], 5672, '/', credentials)
...
class HealthClass(Resource):
    def get(self):
        mongodb = HealthMongoDB(config[0]['mongo']['host'])
        rabbitmq = HealthRabbitMQ(config[1]['rabbit']['host'], config[1]['rabbit']['user'], config[1]['rabbit']['pass'])
        vault = HealthVault()
        consul = HealthConsul()
